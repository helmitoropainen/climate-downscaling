...
# This file was further modified by Helmi Toropainen (2025) for the purpose of downscaling
# climate data
...
class UNet(torch.nn.Module):
    def __init__(self,
        ...
        use_diffuse = 1                     # Use Unet for diffusion
    ):
        super().__init__()
        ...
        if not (self.map_noise is None and self.map_augment is None and self.map_label is None):
            ...
            if self.map_label is not None:
                ...
                if self.training and self.label_dropout:
                    tmp = tmp * (torch.rand([x.shape[0], 1]) >= self.label_dropout).to(
                        tmp.dtype)
                    tmp = tmp.type_as(x)
    ...
    def forward(self, x, sigma, condition_img=None, class_labels=None,
        ...
        if self.label_dim == 0:
            ...
            if class_labels is None:
                class_labels = torch.zeros([1, self.label_dim])
                class_labels = class_labels.type_as(in_img)
            else:
...
#----------------------------------------------------------------------------
# Flow Matching Network adopted from "Flow Matching Guide and Code" 
# with UNet backbone
